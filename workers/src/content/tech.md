...
- **Red Hat OpenShift (OCP)** — proposed and adopted as strategic PaaS direction for XRAN (2023). Detailed work plan authored covering manufacturing, air-gap installation, BBU workloads, networking, security, monitoring. Evaluated OCP EUS Term-2 for stronger security compliance. Technical escalation on memory footprint, performance profiles, CPU management, and workload partitioning.
...
- **Wind River WRCP** — evaluated and deployed for a major APAC operator's 5G scenarios (2021–2022). Kernel roadmap analysis (CentOS 7 / Yocto / Debian), BPF vs Preempt-RT constraints, SR-IOV setup, containerd configuration, and production patching.
...
- **Yocto** — assessed as an alternative bare-metal/immutable OS path for XRAN and DAS/Jazz appliance models (2022).

### Networking

- **DPDK** — evaluated as Ethernet-based data-plane acceleration for XRAN, compared against InfiniBand/RDMA. Later revisited in VPP/fd.io and high-performance IPSec contexts for IPSec and BBU data-plane improvements.
...
- **IPSec / strongSwan** — baseline IPSec implementation for XRAN/5G CU security. Benchmarked at approximately 1.6 Gbps on 10 Gbps link, motivating high-performance IPSec acceleration work.
...
- **Intel E810** — assessed in 2021 and maintained debug sessions through the years until 2024 when mature enough.
...
- **Cisco** — extensive hands-on with NCS 540 timing switches and PTP validation (2020–2022), ASR 920 early timing (2019), lab integration tests in Milan (2019), CBRS project support (2019), Paris lab O&M/NSO integration (2019), NSO/ESC/MANO and NETCONF/YANG NED development discussions.
- **Juniper** — IPsec/Security Gateway integration for a European operator's Open RAN deployment, debugging certificate trust chains and AutoVPN behavior (2021).
...
- **AWS** — EKS/EKS-A/EKS-D evaluation for RAN workloads, Snowball Edge assessment, EKS + Multus PoC, and platform partnership discussions (2021–2023).
...
- **Advantech** — server platform for XRAN and DAS 3.0/Jazz, RHEL9/RHCOS footprint validation (2025–2026).
...
- **Fujitsu** — O-RAN radio integration in operator-facing context, C/U-plane and S-Plane review, SyncE and fronthaul behavior clarification (2021–2022).
- **Mavenir** — O-RU integration analysis for operator macro/O-RAN scenarios, identification of non-standard M-Plane parameters outside O-RAN YANG models (2022).
...
- Worked with Tier-1 operators across APAC, Europe, and North America on XRAN and O-RAN deployment scenarios, including Wind River platform deployment, Security Gateway integration, O-RAN plugfest preparation, timing validation, and third-party O-RU integration.
